...
# @Last Modified time: 2022-06-27 16:39:56
...
def readme_en_us(rule_set_path: str, rule_dict: dict, mdFile: MdUtils):
    ...
    for each_rule in rule_dict["data"]:
        ...
        mdFile.new_paragraph(text="Behavioral Description: When the source process`{src_proc}`initializes the following actions, {action}".format(
            src_proc=each_rule["procname"], action="HIPS module should let the user decide." if int(each_rule["power"]) == 1 else "HIPS module should automatically block."))
        policy_list = list()
        for each_action in each_rule["policies"]:
